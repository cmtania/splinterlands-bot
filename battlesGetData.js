...
const combine = require('./data/combineHistory.js')
const fileDir = `data/history/`
...
//creating file path and filename
var complFileName = fileDir + `history` + dateString + `.json`;
var fileName  = `history` + dateString + `.json`;
...
Promise.all(battles).then(() => {
    ...
    console.log(cleanBattleList.length,'length before removing duplicate');//checking the number of battles retrieved.

    //call the removeDuplicate function
    var filterBattleList = removeDup.removeDuplicate(cleanBattleList,'battle_queue_id');
    console.log(filterBattleList.length,'length after removing duplicate');//checking the number of battles retrieved after removing duplicates 

    fs.writeFile(complFileName, JSON.stringify(cleanBattleList), function (err) {
        if (err) {
            console.log(err);
        } {
            console.log("created " + fileName + " from players: " + JSON.stringify(usersToGrab));
             //call for the updating for main stored procedure
             combine.combineHistory(fileName);
        }
    });
